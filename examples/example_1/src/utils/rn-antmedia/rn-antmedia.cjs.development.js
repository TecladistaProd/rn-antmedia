...
function useAntMedia(params) {
  ...
  var localStream = react.useRef(null);
  var socket = react.useRef({
    ws: null
  }).current;
  var playStreamIds = react.useRef([]).current;
  var remotePeerConnection = react.useRef({}).current;
  var remotePeerConnectionStats = react.useRef({}).current;
  var remoteDescriptionSet = react.useRef({}).current;
  var iceCandidateList = react.useRef({}).current;
  var bandwidth = react.useRef({
    value: bwh || 900
  }).current;
  var adaptorRef = react.useRef(null);
  var closePeerConnection = react.useCallback(function (streamId) {
  ...
  var getVideoSender = react.useCallback(function (streamId) {
  ...
  var changeBandwidth = react.useCallback(function (bw, streamId) {
    try {
      var errorDefinition = '';
  ...
  var iceCandidateReceived = react.useCallback(function (event, streamId) {
    if (event.candidate) {
      var jsCmd = {
  ...
  var onTrack = react.useCallback(function (event, streamId) {
  ...
  var initPeerConnection = react.useCallback(function (streamId, dataChannelMode) {
    try {
      if (remotePeerConnection[streamId] == null) {
        var closedStreamId = streamId;
        remotePeerConnection[streamId] = new reactNativeWebrtc.RTCPeerConnection(peerconnection_config || {
          iceServers: []
        });
  ...
  var initDataChannel = react.useCallback(function (streamId, dataChannel) {
  ...
  var gotDescription = react.useCallback(function (configuration, streamId) {
    try {
  ...
  var startPublishing = react.useCallback(function (streamId) {
    try {
  ...
  var getUserMedia = react.useCallback(function (mdC) {
    try {
      return Promise.resolve(reactNativeWebrtc.mediaDevices.getUserMedia(mdC)).then(function (stream) {
  ...
  var publish = react.useCallback(function (streamId, token, subscriberId, subscriberCode) {
    var data = {};
  ...
  var joinRoom = react.useCallback(function (room, streamId) {
    var data = {
      command: 'joinRoom',
      room: room,
      streamId: streamId
  ...
  var leaveFromRoom = react.useCallback(function (room) {
    var data = {
  ...
  var join = react.useCallback(function (streamId) {
    var data = {
      command: 'join',
      streamId: streamId
    };
  ...
  var leave = react.useCallback(function (streamId) {
    var data = {
      command: 'leave',
      streamId: streamId
    };
  ...
  var play = react.useCallback(function (streamId, token, room) {
  ...
  var stop = react.useCallback(function (streamId) {
    var data = {
      command: 'stop',
      streamId: streamId
    };
  ...
  var handleTurnVolume = react.useCallback(function () {
    if (localStream.current) {
  ...
  var handleTurnCamera = react.useCallback(function () {
    if (localStream.current) {
  ...
  var getStreamInfo = react.useCallback(function (streamId) {
    var jsCmd = {
  ...
  var addIceCandidate = react.useCallback(function (streamId, candidate) {
    try {
  ...
  var takeConfiguration = react.useCallback(function (idOfStream, configuration, typeOfConfiguration) {
    try {
      ...
      return Promise.resolve(initPeerConnection(streamId, dataChannelMode)).then(function () {
        var _temp12 = _catch(function () {
          return Promise.resolve(remotePeerConnection[streamId].setRemoteDescription(new reactNativeWebrtc.RTCSessionDescription({
            sdp: conf,
            type: type
  ...
  var takeCandidate = react.useCallback(function (idOfTheStream, tmpLabel, tmpCandidate, sdpMid) {
    try {
      var streamId = idOfTheStream;
      var label = tmpLabel;
      var candidateSdp = tmpCandidate;
      var candidate = new reactNativeWebrtc.RTCIceCandidate({
        sdpMLineIndex: label,
  ...
  var peerMessage = react.useCallback(function (streamId, definition, data) {
    var jsCmd = {
  ...
  var sendData = react.useCallback(function (streamId, message) {
    // @ts-ignore
    var dataChannel = remotePeerConnection[streamId].dataChannel;
    dataChannel.send(message);
  }, []);
  var signallingState = react.useCallback(function (streamId) {
    if (remotePeerConnection[streamId] != null) {
...
//# sourceMappingURL=rn-antmedia.cjs.development.js.map
